...
namespace Infinity {
    ...
    const std::map<const SubFolder, std::vector<uint32_t>> s_infinityUIList = {
            {ONE_CHAR_CARS, {0x0F4246, 0x0F4247, 0x0F4254, 0x0F4261}},
            {ONE_CHAR_FROZEN, {0x0F4258, 0x0F4259}},
            {ONE_CHAR_MONSTERS, {0x0F4242, 0x0F424A, 0x0F424E, 0x0F4260}},
            {ONE_CHAR_PHINEAS, {0x0F425A, 0x0F425B, 0x0F4264}},
            {ONE_CHAR_PIRATES, {0x0F4243, 0x0F424C, 0x0F424D, 0x0F425F}},
    ...
    static uint32_t InfinityCRC32(const std::array<uint8_t, 16> &buffer) {
        ...
        for (uint32_t i = 0; i < 12; ++i) {
            uint8_t index = uint8_t(ret & 0xFF) ^ buffer[i];
    ...
    bool InfinityBase::CreateFigure(std::string pathName, uint32_t figureNum, uint8_t series) {
        std::ofstream infFile(pathName.c_str(), std::ios::binary);
        if (!infFile) {
            return false;
        }
        std::array<uint8_t, INF_FIGURE_SIZE> fileData{};
        uint32_t firstBlock  = 0x17878E;
        uint32_t otherBlocks = 0x778788;
        for (int8_t i = 2; i >= 0; i--) {
            fileData[0x38 - i] = uint8_t((firstBlock >> i * 8) & 0xFF);
        }
        for (uint32_t index = 1; index < 0x05; index++) {
            for (int8_t i = 2; i >= 0; i--) {
                fileData[((index * 0x40) + 0x38) - i] = uint8_t((otherBlocks >> i * 8) & 0xFF);
            }
        }
        ...
        std::array<uint8_t, 16> figureData = GenerateBlankFigureData(figureNum, series);
        if (figureData[1] == 0x00)
            return false;

        std::array<uint8_t, 16> key = GenerateInfinityFigureKey(sha1Calc);

        std::array<uint8_t, 16> blankBlock = {};

        AES_ctx context;
        AES_init_ctx(&context, key.data());

        AES_ECB_encrypt(&context, figureData.data());
        AES_ECB_encrypt(&context, blankBlock.data());


        memcpy(&fileData[0], uid_data.data(), uid_data.size());
        memcpy(&fileData[16], figureData.data(), figureData.size());
        memcpy(&fileData[16 * 0x04], blankBlock.data(), blankBlock.size());
        memcpy(&fileData[16 * 0x08], blankBlock.data(), blankBlock.size());
        memcpy(&fileData[16 * 0x0C], blankBlock.data(), blankBlock.size());
        memcpy(&fileData[16 * 0x0D], blankBlock.data(), blankBlock.size());

        infFile.write((char *) fileData.data(), fileData.size());
    ...
    std::vector<uint32_t> GetFiguresForFolder(const SubFolder &folder) {
        const auto &it = s_infinityUIList.find(folder);
        if (it != s_infinityUIList.end()) {
            return it->second;
        }
        return {};
    }
    ...
    std::array<uint8_t, 16> InfinityBase::GenerateBlankFigureData(uint32_t figureNum, uint8_t series) {
        std::array<uint8_t, 16> figureData = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                              0x00, 0x00, 0x00, 0x01, 0xD1, 0x1F};

        // Figure Number, input by end user
        figureData[1] = uint8_t((figureNum >> 16) & 0xFF);
        figureData[2] = uint8_t((figureNum >> 8) & 0xFF);
        figureData[3] = uint8_t(figureNum & 0xFF);
        ...
        uint32_t checksum = InfinityCRC32(figureData);
        for (int8_t i = 3; i >= 0; i--) {
            figureData[15 - i] = uint8_t((checksum >> i * 8) & 0xFF);
        }
        return figureData;
    }
